...
class MoondreamModel(nn.Module):
    ...
    def __init__(
        ...
        # The decoders use 2-layer MLPs: dim -> ff_dim -> out_dim
        ff_dim = config.text.ff_dim  # 8192 inner expansion dim
        self.region = nn.ModuleDict(
            {
                ...
                "coord_decoder": nn.ModuleDict(
                    {
                        "fc1": linear_cls(
                            config.region.dim, ff_dim, dtype=dtype
                        ),
                        "fc2": linear_cls(
                            ff_dim, config.region.coord_out_dim, dtype=dtype
                        ),
                    }
                ),
                ...
                "size_decoder": nn.ModuleDict(
                    {
                        ...
                        "fc2": linear_cls(
                            ff_dim, config.region.size_out_dim, dtype=dtype
                        ),
                    }
                ),
            }
    ...
    def _run_vision_encoder(self, image: Image.Image) -> torch.Tensor:
        all_crops, tiling = prepare_crops(
            image, self.config.vision, device=self.device,
            dtype=self.vision.pos_emb.dtype,
        )
