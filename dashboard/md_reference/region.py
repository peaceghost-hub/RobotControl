...
def decode_coordinate(hidden_state: torch.Tensor, w: nn.Module) -> torch.Tensor:
    ...
    """
    x = w.coord_decoder.fc1(hidden_state)
    return w.coord_decoder.fc2(torch.nn.functional.gelu(x, approximate="tanh"))
...
def decode_size(hidden_state: torch.Tensor, w: nn.Module) -> torch.Tensor:
    ...
    """
    x = w.size_decoder.fc1(hidden_state)
    return w.size_decoder.fc2(torch.nn.functional.gelu(x, approximate="tanh")).view(2, -1)
