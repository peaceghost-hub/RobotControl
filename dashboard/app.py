...
@app.route('/api/compass_correction')
def get_compass_correction():
    """Serve compass correction metadata for the dual-compass dashboard.

    Navigation uses the Pi's corrected heading directly. The dashboard uses
    this metadata for status labelling and as a fallback when only magnetic
    heading is available. Returns the JSON generated by compass_diagnostic.py
    plus the configured declination.
    """
    correction_path = os.path.join(
        os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
        'raspberry_pi', 'calibration', 'compass_correction.json'
    )
    ...
    try:
        ...
        if os.path.exists(config_path):
            ...
            declination_deg = (
                config_data.get('compass', {})
                .get('calibration', {})
                .get('declination_deg', declination_deg)
            )

        if os.path.exists(correction_path):
            with open(correction_path, 'r') as f:
                data = json.load(f)
            if 'declination_deg' not in data:
                data['declination_deg'] = declination_deg
            return jsonify(data), 200
        else:
            return jsonify({'type': 'none', 'offset': 0, 'declination_deg': declination_deg}), 200
    except Exception as e:
        logger.warning("Failed to load compass correction: %s", e)
        return jsonify({'type': 'none', 'offset': 0, 'declination_deg': -0.5}), 200
...
def ai_drive_toggle():
    ...
    This toggle no longer sends AI_OVERRIDE to the Mega — obstacle
    avoidance is handled automatically by the Pi + AI Vision."""
    data = request.get_json(silent=True) or {}
    enabled = bool(data.get('enabled', False))
    ai_vision.set_auto_drive(enabled)

    logger.info("Auto-drive toggled: %s", 'ON' if enabled else 'OFF')
